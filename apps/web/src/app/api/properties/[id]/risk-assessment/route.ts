...
async function calculateRiskScore(propertyId: string): Promise<number> {
  ...
  // High-value claims
  type InsuranceClaim = { amount: number | null }
  const highValueClaims = property.insuranceClaims.filter((c: InsuranceClaim) => c.amount != null && c.amount > 20000)
  riskScore += highValueClaims.length * 5

  // Frequent repairs
  type WorkEvent = { workDate: Date }
  const recentWorkEvents = property.workEvents.filter((w: WorkEvent) => {
  ...
  // Rental history
  type RentalSignal = { startDate: Date | null; endDate: Date | null }
  const rentalYears = property.rentalSignals.reduce((total: number, rental: RentalSignal) => {
