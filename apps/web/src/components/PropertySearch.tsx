    ...
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error('[PropertySearch] Autocomplete response not OK:', response.status, errorText)
          setSuggestions([])
          setShowSuggestions(false)
          return
        }
        ...
        console.log('[PropertySearch] Autocomplete response:', { query: trimmedQuery, count: suggestionList.length, suggestions: suggestionList })
        setSuggestions(suggestionList)
        setShowSuggestions(suggestionList.length > 0)
      } catch (error) {
        if ((error as Error).name !== 'AbortError') {
          console.error('[PropertySearch] Autocomplete fetch error:', error)
          setSuggestions([])
          setShowSuggestions(false)
        }
      } finally {
