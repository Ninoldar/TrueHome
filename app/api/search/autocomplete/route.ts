...
export async function GET(request: NextRequest) {
  try {
    ...
    const suggestions = properties.map((p: { id: string; address: string; city: string; state: string; zipCode: string }) => ({
      id: p.id,
      label: `${p.address}, ${p.city}, ${p.state} ${p.zipCode}`,
      address: p.address,
      city: p.city,
      state: p.state,
      zipCode: p.zipCode,
    ...
    console.log(`[Autocomplete] Query: "${rawQuery}", Found ${suggestions.length} suggestions`)
    return NextResponse.json({ suggestions })
  } catch (error) {
    console.error('[Autocomplete] Error:', error)
    return NextResponse.json(
      { error: 'Failed to get autocomplete suggestions', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
